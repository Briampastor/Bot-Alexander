let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, 'Bot Arrow.png', '*✨Arrow_OG✨*\n*BY:* ✨𝐁𝐨𝐭 𝐀𝐫𝐫𝐨𝐰✨', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}
handler.help = ['Bot-Arrow texto']
handler.tags = ['tools']
handler.command = /^((Bot-Arrow)?B-Arrow)$/i
handler.limit = true
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('Bot', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('Arrow', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
